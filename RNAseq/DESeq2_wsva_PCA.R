...
library(ggforce)
library(stringr)
cnts <- read.table("counts_Plecuc1.txt", sep='\t',header=T,row.names=1,check.names=F)
cnts$Length <- NULL
#coldata <- read.table("coldata_together.txt", sep='\t',header=T,row.names=1,check.names=F)
coldata <- read.table("coldata_nomock.txt", sep='\t',header=T,row.names=1,check.names=F)
...
for (k in 2:svnum){
	...
	#group1idx <- sampleIdx[colnames(rldData) %in% c("pad4_Mock_1hr_rep1","pad4_Mock_1hr_rep2","pad4_Mock_1hr_rep3","pad4_Thio_1hr_rep1","pad4_Thio_1hr_rep2","pad4_Thio_1hr_rep3","WT_Mock_1hr_rep1","WT_Mock_1hr_rep2","WT_Mock_1hr_rep3","WT_Thio_1hr_rep1","WT_Thio_1hr_rep2","WT_Thio_1hr_rep3")]
	#group2idx <- sampleIdx[colnames(rldData) %in% c("pad4_Mock_6hr_rep1","pad4_Mock_6hr_rep2","pad4_Mock_6hr_rep3","pad4_Thio_6hr_rep1","pad4_Thio_6hr_rep2","pad4_Thio_6hr_rep3","WT_Mock_6hr_rep1","WT_Mock_6hr_rep2","WT_Mock_6hr_rep3","WT_Thio_6hr_rep1","WT_Thio_6hr_rep2","WT_Thio_6hr_rep3")]
	group <- c("all")#,"pad4_Thio_1hr","pad4_Thio_6hr")#"all","subset1","subset2")
	top <- 500
	allgn <- length(rownames(rldData))
	allidx <- list(sampleIdx)#,group1idx,group2idx)#sampleIdx)#allsampleIdx,group1idx,group2idx)
	for(i in 1:length(allidx)){
		...
		#for top 500 genes that contribute the most to the variance of PC1 and PC2.
			...
			for (fontsize in c(6)){#(5,6,7)){
			ggplot(pcaData, aes(x = PC1, y = PC2, colour = Group, label = ID)) +
				geom_point(size = 3) +
				xlab(paste0("PC1: ",percentVar[1],"% variance")) +
				ylab(paste0("PC2: ",percentVar[2],"% variance")) +
				geom_mark_ellipse(aes(color = Group, label=Group), label.fontsize=fontsize*2.8,show.legend=FALSE) +
				#scale_colour_manual(name = 'SynCom',values = cols[colorIdx]) +
				stat_ellipse(aes(x = PC1, y = PC2, group = Group), type = 't', linetype = 2, level = 0.8,show.legend=FALSE) +
				coord_fixed(1) +
				theme_classic() +
				geom_text(aes(label=ID),vjust=2,size=fontsize,show.legend=FALSE) +
			...
			if(n == allgn){
				...
				#ggsave(outpdf, width=18,height=18)
			} else {
				outpdf <- paste(group[i],"_PCA_top",n,"_sva",k,"_fontS",fontsize,".pdf",sep="")
				#ggsave(outpdf, width=18,height=18)
			}
			}
			#outjpg <- paste(group[i],"_PCA_top500.jpg",sep="")
			#ggsave(outjpg, width = size,height=size)
		}
...
for(i in 1:length(contrasts)){
    ...
    outf <- paste("evolvedPlecuc_",ctrl,"VS",trt,"_diff.txt",sep="")

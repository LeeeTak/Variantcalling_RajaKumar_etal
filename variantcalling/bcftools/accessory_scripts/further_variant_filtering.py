...
def repeatchk(pattern,string):
    ...
    for c in pattern:
        cc += c
        rpttimes = len(pattern) // len(cc)
        comp = cc*rpttimes
        if len(pattern) > len(comp):
            comp = comp+pattern[len(comp):]
        if pattern == comp:
            upattern = cc
            break
    pcount = 0
    N = len(upattern)
    strings = re.findall(N*".",string)
    for s in strings:
        if s == upattern:
            pcount += 1
        else:
...
def getflank(cm,p):
    ...
    cmd = "samtools faidx /netscratch/dep_psl/grp_rgo/taklee/Ram_variantcalling/hybrid/Plecuc1_hybrid_assembly.fasta "+cm+":"+str(ps)+"-"+str(pe)
...
def merge_chkrep_vcfs(inpath,outpath):
    ...
    cmd = "bcftools merge "+mergeinputs+" -o "+merged
        ...
        for l in open(merged,"r"):
            ...
            if not re.match("#",l):
                ...
                if rptchk == "dontchk":
                    ...
                    chkseq = rptchk.upper()+"".join(j for j in flseq[flnklen+reflen:])
                    checked = repeatchk(rptchk,chkseq)
                    if checked:
                        cnts_rpt[chnge] = cnt
                    else:
                        cnts_norpt[chnge] = cnt
            ...
            for l in open(f,"r"):
                ...
                if re.match("#",l):
                    ...
                    if al not in repeats:
                        ll = l.replace("_pilon","")
                        norepfile.write(ll.strip()+"\n")

#remove ancestral variants
def filter_Ancestral(inpath,outpath,ancestral):
    inputfiles=glob.glob(inpath+"/*.gz")
    subprocess.run("tabix -p vcf "+ancestral,shell=True)
    for f in inputfiles:

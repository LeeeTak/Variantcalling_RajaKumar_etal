...
FILES=$(cat /netscratch/dep_psl/grp_rgo/taklee/Ram_variantcalling/haplotypecaller/samples.list)
mkdir -p filt3_hardfiltered
for F in $FILES;
do
    infile=${F}_hybrid_rm_scmap130_chim_multi_bcftools_Ancestralfilt_repeatfilt.vcf
	...
	-V $input \
	-filter "QUAL < 6" --filter-name "QUAL6" \
